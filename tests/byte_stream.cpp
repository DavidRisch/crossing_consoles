...
TEST(ByteServer, SuccessfulNoneBlocking) {
  ...
  std::optional<SocketByteStream> client_byte_stream;

  std::thread client_thread([&client_byte_stream] { client_byte_stream = SocketByteStream::CreateClientSide(); });

  std::optional<SocketByteStream> server_byte_stream;

  int counter = 1000;
  while (!server_byte_stream.has_value() && counter > 0) {
  ...
  ASSERT_NE(counter, 0);
  ASSERT_TRUE(server_byte_stream.has_value());

  client_thread.join();
  ASSERT_TRUE(client_byte_stream.has_value());
